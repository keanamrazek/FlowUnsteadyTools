...
domainLimits = 10.0^2 #simulation domain radius

gammaCrit = 0.005^2  #minimum vorticity

gammaCritup = 8.0^2 #maximum vorticity
...
RPMcontrol(t) = 1.0*(t>0.1) + 10*t*(t<=0.1)
...
elseif fidelityLevel=="Low"
    n_rotor         = 13                  # Number of blade elements per blade
...
uns.run_simulation(
    ...
    vlm_init        = true,            # Initialize the first step with the VLM semi-infinite wake solution
