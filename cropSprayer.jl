...
bulkDataSavePath = "/dumpHDD1/VPM/coAxial/"*caseName
...
temperature = 30.0 #deg C ground temperature

altitude = 2000.0 #m

rhoOverride = nothing
muOverride = nothing
temperatureOverride = 25

nrevs    = 3 # Number of revolutions in simulation

domainLimits = 8.0^2 #simulation domain radius
...
bladeFile      = ["qx6_blade.csv", "qx6_blade.csv"]       # Blade geometry
...
for ri in 1:numberOfRotors
    ...
    pitchDist = [pitchDist."r/R" pitchDist."twist (deg)".+collective[ri]]
    sweepDist = [sweepDist."r/R" sweepDist."y/R"]
    heightDist = [heightDist."r/R" heightDist."z/R"+sind(coningAngle[ri])*heightDist."r/R"]
...
if plotResults
    ...
    for ri in 1:numberOfRotors-1
        ...
        plot!(plt1, df.T, CT, label="rotor $ri")

        plot!(plt2, df.T, CQ, label="rotor $ri")
        ...
        y = y[x.<12]
        x = x[x.<12]
        plot!(plt4, x, y, label="rotor $ri")

        plot!(plt5, df.T, T, label="rotor $ri")

        plot!(plt6, df.T, Q, label="rotor $ri")
    ...
    nrows = 3
    ncols = 2
    plt = plot(plt1, plt2,plt3, plt4,plt5,plt6, layout=(nrows,ncols), size=(ncols*700.0, nrows*600.0), legend=false)
